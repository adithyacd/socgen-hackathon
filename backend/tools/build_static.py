...
def render_report(ctx: AnalysisContext) -> str:
    ...
    top_cves = notable_cves(ctx)
    hero = war_room_impact(ctx, top_cves[0].cve_id) if top_cves else None
    if hero is not None:
        hero.narrative = incident_brief(hero)
...
<style>
  * {{ box-sizing: border-box; }}
  ...
  <div class="grid">
    <div class="kpi"><div class="n" style="color:#c81e3a">{r.summary.exploitable_criticals}</div><div class="l">High-priority criticals</div></div>
    <div class="kpi"><div class="n">{ex['actionable']}/{ex['total_vuln_alerts']}</div><div class="l">Actionable vuln alerts (high/med exploitability)</div></div>
    <div class="kpi"><div class="n" style="color:#1f8a6d">{round(m['overall']['recall']*100)}%</div><div class="l">Detection recall vs. labels</div></div>
  ...
  <h2>Zero-day exposure — {hero.cve.cve_id} ({hero.cve.affected_library})</h2>
  <div class="brief">{hero.narrative}</div>
  ...
  <table><tbody>
    <tr><td>Vulnerability detection recall</td><td>{round(det['vulnerability']['recall']*100)}%</td></tr>
    <tr><td>Transitive resolution</td><td>{round(det['transitive_resolution']['recall']*100)}%</td></tr>
    <tr><td>License-conflict detection</td><td>{round(det['license']['recall']*100)}%</td></tr>
    <tr><td>Maintenance detection</td><td>{round(det['maintenance']['recall']*100)}%</td></tr>
    <tr><td>Overall recall / precision (is_risky)</td><td>{round(m['overall']['recall']*100)}% / {round(m['overall']['precision']*100)}%</td></tr>
    <tr><td>Actionable vuln alerts (high/med exploitability)</td><td>{ex['actionable']} of {ex['total_vuln_alerts']} ({round(ex['actionable_pct']*100)}%)</td></tr>
  </tbody></table>
